...
class GetRelatedSessionBySpecificSubjectUseCase(
    private val sessionRepository: SessionRepository,
    //we recommend injecting Dispatchers For easier testing
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO
) {
    suspend operator fun invoke(subjectId: Int): Flow<List<Session>> {
        return withContext(ioDispatcher) {
            sessionRepository.getRelatedSessionBySpecificSubject(subjectId = subjectId)
        }
    }
}