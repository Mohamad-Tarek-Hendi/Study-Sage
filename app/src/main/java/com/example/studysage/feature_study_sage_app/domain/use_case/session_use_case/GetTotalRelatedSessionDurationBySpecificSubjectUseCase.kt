...
class GetTotalRelatedSessionDurationBySpecificSubjectUseCase(
    private val sessionRepository: SessionRepository,
    //we recommend injecting Dispatchers For easier testing
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO
) {
    suspend operator fun invoke(subjectId: Int): Flow<Long> {
        return withContext(ioDispatcher) {
            sessionRepository.getTotalRelatedSessionDurationBySpecificSubject(subjectId = subjectId)
        }
    }
}