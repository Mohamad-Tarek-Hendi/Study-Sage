...
class GetRelatedCompletedTasksBySpecificSubjectUseCase(
    private val taskRepository: TaskRepository,
    //we recommend injecting Dispatchers For easier testing
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO
) {
    suspend operator fun invoke(subjectId: Int): Flow<List<Task>> {
        return withContext(ioDispatcher) {
            taskRepository.getRelatedCompletedTasksBySpecificSubject(subjectId = subjectId)
        }
    }
}